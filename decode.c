...
// stores opcodes into return parameter
void decode_comp(char *d_ins, char *ins) {
	...
	fprintf(stderr,"error in comp field, unknown operation\n");
...
void decode_dest(char *d_ins, char *ins) {
	...
	while(ins[i]) {
		...
		case 'D':
			...
			break;
		default:
		fprintf(stderr,"error in destination field, unknown dest %c",ins[i]);
		fprintf(stderr," in statement %s\n",ins);
		exit(3);
		}
		++i;
...
void decode_jmp(char *d_ins,char *ins) {
	...
	if(are_equal(ins,jmp_pairs[rnk].mnem)) {
		...
		fprintf(stderr,"error in jump field, unknown directive");
		fprintf(stderr," in statement %s\n",ins);
		exit(3);
	}
